...
def keyd_config(swap: bool = False, voice: bool = False) -> str | None:
    # Three mapped states, plus a neutral state that ships no file at all.
    # Voice off must not leave capslock = f24 in place.
    #
    # Neutral returns None rather than an identity mapping: keyd applies
    # exactly one config per device, and this file's name sorts after a
    # hand-written /etc/keyd/default.conf, so an installed `capslock =
    # capslock` wins the match and silently voids every mapping the user
    # maintains for that keyboard -- the opposite of the no-op it reads as.
    if swap and voice:
...
def keyd_config_matches(swap: bool, voice: bool) -> bool:
    expected = keyd_config(swap, voice)
    if expected is None:
        # Neutral: correct means our file is absent, so keyd falls back to
        # whatever config the user maintains for the device.
...
def remap_is_live(swap: bool, voice: bool) -> bool:
    if keyd_config(swap, voice) is None:
        # Nothing of ours is mapped, so whether keyd runs is not ours to
        # require: it may be serving configs that have nothing to do with us.
...
def _apply_keyd_unlocked(swap: bool, voice: bool, *, force: bool = False) -> None:
    ...
    if config is None:
        # Uninstall rather than install a passthrough -- see keyd_config().
        # keyd keeps running for whatever else it serves; it only needs a
        # restart to notice our file is gone. Skip the privileged call
        # entirely when there is nothing left to remove, so a forced sync
        # into the neutral state cannot raise a pointless pkexec prompt.
